...
export const PersonnelSchema = z.object({
    avatar: z.string().optional().default("/images/default-avatar.png"),
    name: z.string({message: "Введите ФИ"}).min(3, {message: "ФИ слишком короткое"}).max(255, {message: "ФИ слишком длинное"}),
    ...
    summary: z.string({message: "Введите краткое резюме"}).min(20, {message: "Минимум 20 символов)"}).max(1000, {message: "Резюме слишком длинное"}),
    ...
    period: z.string({message: "Укажите период обучения (например, 2022-2026)"}).length(9),
